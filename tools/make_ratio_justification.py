...
w("")
w("**We are running the motor past its rev limiter on the fastest part of the lap, and 4.3 is")
w("the highest ratio that stops that.** Everything else is secondary.")
w("")
...
w("")
w("## 2. Why 4.3 specifically, and not 4.4")
w("")
w("Rolling radius sets how fast the motor spins for a given road speed, and a loaded tyre is")
w("smaller than a free one. `params_cfr26` flags our rolling radius as **not measured under")
w("load**, so the honest thing is to check both ends of the plausible range.")
w("")
w("**Highest ratio that still clears 6000 rpm:**")
w("")
w("| Rolling radius | Max ratio |")
w("|---|---|")
w("| Free 0.200 m | 4.5471 |")
w("| Mid 0.195 m | 4.4334 |")
w("| **Loaded 0.190 m** | **4.3197** |")
w("")
w("On the conservative radius the ceiling is **4.32**. That is the whole answer to why 4.3 and")
w("not 4.4: **4.4 is above the ceiling on the loaded radius.** We checked the entire 4.40 to")
w("4.45 band and every single buildable combination fails this, including the ones that fit the")
w("chassis perfectly.")
w("")
w("4.3 is not a compromise pick. It is the **highest ratio that survives the worst-case radius**,")
w("which means it is the most performance we can take without betting on a number nobody has")
w("measured.")
w("")
w("## 3. What it does to the motor speed")
w("")
w("| Ratio | Max rpm, free radius | Max rpm, loaded radius |")
w("|---|---|---|")
w("| 4.6154 (now) | 6090 **over** | 6411 **over** |")
w("| 4.4615 | 5887 | 6197 **over** |")
w("| **4.3077** | **5684 OK** | **5983 OK** |")
w("| **4.2857** | **5655 OK** | **5953 OK** |")
w("")
...
w("### Which one")
w("")
w("**14T / 30T if we can source a 14T in the spline bore.** It keeps the 30T driven we already")
w("have, keeps the 42-pitch chain we already have, moves the axle 1.34 mm, and cuts chain")
w("tension 7% and chordal ripple 14% because the driver is bigger.")
w("")
w("**13T / 28T if we cannot.** Needs a new 28T driven, a 40-pitch chain (two links out), and")
w("moves the axle 2.09 mm. Chain loads stay exactly as they are today. It also frees 15 mm of")
w("rear envelope, which is the most room of any option.")
w("")
...
w("| # | Item | Who |")
w("|---|---|---|")
w("| 1 | Measure the loaded rolling radius | anyone with a tape and a loaded tyre |")
...
w("")
w("Item 1 is worth doing regardless: if the loaded radius comes back at 0.195 or better, the")
w("ceiling moves to 4.43 and 14T/31T at 4.4286 becomes the best config in the whole study.")
