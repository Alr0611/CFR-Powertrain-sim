...
% ---- LOCKED 4.3077 (13/28) vs PREVIOUS 4.61 (13/30): the decision, read out directly ----
[~,iLock] = min(abs([resultsByRatio.ratio] - 4.3077));
[~,iPrev] = min(abs([resultsByRatio.ratio] - 4.61));
fprintf('\n--- LOCKED 4.3077 (13/28) vs PREVIOUS 4.61 (13/30) ---\n');
fprintf('  4.3077 (LOCKED): AvgEff %5.1f%% | HiEff %5.1f%% | FinalSOC %5.2f%% | 98%%-start %5.1f%%\n', ...
    resultsByRatio(iLock).avg_eff, resultsByRatio(iLock).hi_eff, resultsByRatio(iLock).SOC, resultsByRatio(iLock).SOC98);
fprintf('  4.61   (prev)  : AvgEff %5.1f%% | HiEff %5.1f%% | FinalSOC %5.2f%% | 98%%-start %5.1f%%\n', ...
    resultsByRatio(iPrev).avg_eff, resultsByRatio(iPrev).hi_eff, resultsByRatio(iPrev).SOC, resultsByRatio(iPrev).SOC98);
fprintf('  delta (locked - prev): dSOC %+.2f%% | dHiEff %+.1f%% | d(98%%-start) %+.2f%%\n', ...
    resultsByRatio(iLock).SOC -resultsByRatio(iPrev).SOC, ...
    resultsByRatio(iLock).hi_eff-resultsByRatio(iPrev).hi_eff, ...
    resultsByRatio(iLock).SOC98-resultsByRatio(iPrev).SOC98);
